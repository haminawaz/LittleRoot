      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
        {isLoading ? (
          ...
          <div className="space-y-6">
            {/* Trial Warning */}
            {isTrial &&
              user?.daysLeftInTrial !== undefined &&
              user.daysLeftInTrial <= 3 && (
                <Alert data-testid="alert-trial-ending">
                  <AlertCircle className="h-4 w-4" />
                  <AlertDescription>
                    Your free trial ends in {user.daysLeftInTrial}{" "}
                    {user.daysLeftInTrial === 1 ? "day" : "days"}. Upgrade now
                    to continue creating beautiful children's books!
                  </AlertDescription>
                </Alert>
              )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <p
                      ...
                      data-testid="text-books-remaining"
                    >
                      {isTrial
                        ? (user?.booksLimitPerMonth || 0) -
                          (user?.booksUsedThisMonth || 0)
                    ...
                    >
                      ...
                      <Progress
                        value={
                          (((user?.templateBooksLimit || 0) -
                            (user?.templateBooksRemaining || 0)) /
                            (user?.templateBooksLimit || 1)) *
                          100
                        }
                        className="h-2"
                      />
                      <p className="text-xs text-muted-foreground mt-2">
                        {user?.templateBooksLimit &&
                        user?.templateBooksRemaining
                          ? `${(
                              ((user.templateBooksLimit -
                                user.templateBooksRemaining) /
                                user.templateBooksLimit) *
                      ...
                      <Progress
                        value={
                          (((user?.bonusVariationsLimit || 0) -
                            ...
                            (user?.bonusVariationsLimit || 1)) *
                          100
                        }
                        className="h-2"
                      />
                      <p className="text-xs text-muted-foreground mt-2">
                        {user?.bonusVariationsLimit &&
                        user?.bonusVariationsRemaining
                          ? `${(
                              ((user.bonusVariationsLimit -
                                user.bonusVariationsRemaining) /
                                user.bonusVariationsLimit) *
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    {user?.hasCommercialRights ? (
                      ...
                      >
                        ...
                        <span className="text-sm font-medium">
                          {user?.hasResellRights
                            ? "Commercial Rights + Resell Rights"
                            : "Commercial Rights (Personal Publishing)"}
                        </span>
                      </motion.div>
                    ) : (
                      <motion.div
                      ...
                      >
                        ...
                        <span className="text-sm text-muted-foreground">
                          No commercial rights · Upgrade for commercial use
                        </span>
                      </motion.div>
                    )}
                  </div>
                          ...
                          <AlertDialogContent data-testid="dialog-cancel-subscription">
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                ...
                                end of your current billing period. You'll
                                continue to have access to all features until
                                then, but you won't be charged again.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                    ...
                    {user?.cancelAtPeriodEnd && (
                      <Alert data-testid="alert-subscription-canceling">
                        ...
                        <AlertDescription>
                          ...
                          current billing period. You can continue to use all
                          features until then.
                        </AlertDescription>
                      </Alert>
                    )}
                  ...
                  <div className="pt-4 border-t border-border">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm" data-testid="text-trial-days">
                        ...
                        {user.daysLeftInTrial === 1 ? "day" : "days"} remaining
                        in trial
                      </span>
                    </div>
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {availablePlans.map((plan) => {
                  ...
                  if (plan.resellRights) {
                    ...
                    commercialRightsText =
                      "Full Commercial Rights (personal publishing)";
                  }
                          ...
                          <div className={`relative overflow-hidden ${style.gradientBarClasses} rounded-lg px-4 py-2.5 shadow-sm`}>
                            <div className="relative flex items-center justify-center gap-2">
                              ...
                              <span className="text-sm font-bold text-white tracking-wide">
                                Save {promotion.discountPercent}% on this plan
                              </span>
                            </div>
              ...
              <Card data-testid="card-billing-info">
                ...
                <CardContent className="space-y-4">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Payment management coming soon. Contact support for billing
                    changes.
                  </p>
                </CardContent>
              </Card>
